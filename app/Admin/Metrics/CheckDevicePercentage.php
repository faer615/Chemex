...
class CheckDevicePercentage extends Card
{
    ...
    public function content($content)
    {
        ...
        $check_record = CheckRecord::where('check_item', 'device')->where('status', 1)->first();
        if (!empty($check_record)) {
            $check_tracks_counts = CheckTrack::where('check_id', $check_record->id)
                ->where('status', '!=', 0)
                ->get()
                ->count();
            $done_counts = ($device_records_all - $check_tracks_counts) . ' / ' . $device_records_all;
            $percentage = ($device_records_all - $check_tracks_counts) / $device_records_all;
        } else {
